...
void update_index_run(int n_process, process_t *process_arr[], int size)
{
    ...
    if (process_arr[n_process] -> run[size + 1] > 0)
    {
        process_arr[n_process] -> run[size + 1] = 0;
    }
    else if (process_arr[n_process] -> run[size + 1] = 0)
    {
        (process_arr[n_process] -> run[size]) = 0;
    }
...
void update_index_blocked(int n_process, process_t *process_arr[], int size)
{
    ...
    if (process_arr[n_process] -> blocked[size] > 0)
    {
        process_arr[n_process] -> blocked[size] = 0;
    }
    else if (process_arr[n_process] -> run[size + 1] = 0)
    {
        (process_arr[n_process] -> run[size]) = 0;
...
void rr(int n_process, process_t *process_arr[])
{
    ...
    {
        printf("RUN: %d %d %d\n", process_arr[3]->run[0], process_arr[3]->run[1], process_arr[3]->run[2]);
        printf("Blocked: %d %d %d\n", process_arr[3]->blocked[0], process_arr[3]->blocked[1], process_arr[3]->blocked[2]);
        if (!empty(run) || !empty(ready) || !empty(blocked))
        {
            ...
            if (process_arr[find_PID( run->array[0], process_arr, n_process)]->run[0] == 0 || quantum == 2)
            {
                if (process_arr[find_PID( run->array[0], process_arr, n_process)]->run[0] != 0 && quantum == 2)
                {
